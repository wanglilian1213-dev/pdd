-- 文章修改：记录 GPT-5.5 article_detection 识别出的主文章文件名
--
...
-- 主文章靠 GPT-5.5 article_detection stage 识别。识别结果存到这个字段，
-- executeRevision 调 GPT 时再读出来，告诉 GPT "这是主文章，那些是参考"。
--
-- 兼容性：NOT NULL DEFAULT '{}'，旧 revisions 数据自动 backfill 成空数组。
-- executeRevision 见空数组时走旧 prompt 行为（不告诉 GPT 哪份是主文章），
-- 不破坏已落库的历史任务。
...
COMMENT ON COLUMN revisions.main_article_filenames IS
  '由 GPT-5.5 article_detection stage 识别出的主文章文件名列表。executeRevision 用它告诉 GPT 改哪份。空数组 = 旧任务兼容，走旧 prompt 行为。';
