...
function safeModelFacingFilename(filename: string) {
...
export async function prepareScoringMaterialParts(
  ...
  for (const file of files) {
    ...
    const filename = safeModelFacingFilename(file.original_name);

    parts.push({
      type: 'input_text',
      text: `材料文件：${filename}（预判角色：${hintedRole}）`,
    });
    ...
    if (DOCX_EXTENSIONS.has(ext)) {
      ...
      parts.push({
        ...
        text:
          text && text.trim()
            ? `<<<BEGIN DOCX CONTENT OF ${filename}>>>\n${text}\n<<<END DOCX CONTENT>>>`
            : `[DOCX ${filename} 提取失败，可按文件名预判处理]`,
      });
      continue;
    }
    ...
    if (TEXT_EXTENSIONS.has(ext)) {
      parts.push({
        type: 'input_text',
        text: `<<<BEGIN TEXT CONTENT OF ${filename}>>>\n${buffer.toString('utf8')}\n<<<END TEXT CONTENT>>>`,
      });
      continue;
    }
