...
- [x] 正文第一步补上超时兜底，避免任务无限卡在 `writing`，并把这一步单独放宽到 10 分钟
- [x] 后端跨域白名单
- [x] 后端基础安全中间件
- [x] 依赖安全检查 GitHub 工作流
...
| 日期 | 更新内容 |
|------|----------|
| 2026-03-31 | 交接前又做了一轮真实线上验收：直接用桌面上那 3 个任务要求 PDF 新建任务，确认最新主线 `2f20cca` 下，正式题目会优先贴着真实任务主题，不再被 `Report Marking Criteria.pdf` 带偏；前端、后端、清理服务线上状态都正常 |
