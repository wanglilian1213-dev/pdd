...
> 最后更新：2026-04-19 (Landing 落地页重设计 + Academic-RLHF™ 叙事重塑)
>
...
| 日期 | 更新内容 |
|------|----------|
| 2026-04-19 | Landing 落地页整体重设计 + 文案叙事重塑：(1) 以 `Academic-RLHF™` 独家算法为品牌主线贯穿全站（Hero 胶囊、Hero 副标题、对比表表头、算法特质区、证言区、功能卡、Final CTA 共 7 处挂名），不再直接叫卖"查不出 AI / 低 AI 痕迹"等敏感措辞，改为"学术写作该有的样子 → 算法对齐的专业笔触"间接表达；(2) Hero 三根柱子从"2% AI 痕迹 / 0 篇伪造文献 / 90 分交付线"改为"98 分自然语感(RLHF 对齐) / 100% 真实学术文献(Crossref 核验) / 0 返工(Outline-First)"——第三根柱子换角度改成过程承诺，避免"90 分学术线"吹过头；(3) 新增「为什么选拼代代」对比表（通用大模型 / 普通写作工具 / 拼代代）、三大承诺深度区（算法打磨进度条动画 / 引用核验报告样例卡 / 大纲锁定交互）、黑底 3 步流程区、featured + 6 条用户证言区、三类人群定位卡（接单个人 / 小型工作室 / 学生）、9 项功能矩阵、4 档激活码包 + 按字定价、FAQ 手风琴、黑底 Final CTA；(4) 保留 Navbar / Footer 硬编码的 5 个 anchor ID（`#features` / `#cases` / `#feedback` / `#faq` / `#contact-sales`）避免死链；(5) `src/index.css` 底部追加 Landing-only CSS utilities（`grid-bg` / `dark-grid-bg` / `text-gradient-red` / `card-lift` / `marquee` / `animate-landing-float` / `animate-landing-pulse-soft`），Tailwind v4 JIT 全部 class 名用字面量避免拼接导致扫不出；(6) `Footer.tsx` 同步更新品牌描述（"专为学术写作场景打造的专业平台"）与免责声明（"智能写作辅助"替代"自动写作服务"）；(7) 复用现有 `Navbar` / `Footer` / `CustomerSupportPanel` / `Button` / Lucide 图标库，新 Landing.tsx 拆为 9 个独立 section 函数组件便于后续维护；`npm run lint`(tsc --noEmit) + `npm run build` 全部通过，0 error |
| 2026-04-18 | 检测 AI / 降 AI 二合一独立页面（`/dashboard/ai-tools`）上线：顶部 Tab 切换，两条独立功能线（检测 AI + 独立降 AI）可并行运行。检测走 Undetectable.ai Detector REST API（POST /detect + /query 轮询）一次调用返回 8 家主流检测器聚合分数（GPTZero / OpenAI / Copyleaks / Sapling / Writer / ContentAtScale / ZeroGPT / CrossPlag），单价 0.05 积分/字，最低 200 词、最高 30,000 词；独立降 AI 复用现有 Undetectable Humanization client（v11sr + More Human + University + Essay），单价复用 `humanize_price_per_word = 0.4`，最低 500 词；migration 20260418000001 新建 4 张表（`ai_detections` / `ai_detection_files` / `standalone_humanizations` / `standalone_humanization_files`）+ 1 条 `ai_detection_price_per_word = 0.05` 配置 + 4 个 cleanupRuntime 清理函数（卡死回收 + 过期文件清理）；Tasks 页扩成 5 tab（新增「AI 检测记录」「独立降 AI 记录」）；前端柱状图用纯 CSS，不引入 recharts；关键 UX：Undetectable 返回的 `result_details.scoreXxx` 是"人工编写 %"方向和 `result` 相反，前端统一换算 `100 - scoreXxx` 再画柱保证方向一致；并发锁对齐 scoring，部分唯一索引覆盖 `initializing` + `processing`；新增 22 条后端单测；侧边栏菜单新增「检测 AI / 降 AI」入口，图标 Bot |
